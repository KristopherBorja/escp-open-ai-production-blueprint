...
The deployment workflow listens for a successful `Verify` run caused by a push to `main`, checks out that run's exact commit, and syncs only that SHA to one public Static Space. A repository-owned script performs the Git push without putting the environment-scoped token in the remote URL. A rollback is:
